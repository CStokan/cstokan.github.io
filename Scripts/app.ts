...
(function()
{
    ...
    function CheckLogin(): void
    {
        ...
        if(sessionStorage.getItem("user"))
        {
            ...
            // If users logged in show the nav
            $("#task-list").show();
            // If task list is clicked change to active and add navigation events
            $("#task-list").on("click", function()
            {
                
                $("#task-list").html(
                    `<a class="nav-link active" data="task-list"><i class="fas fa-pen"></i> Task-List</a>`
                );
                AddNavigationEvents();
            });
